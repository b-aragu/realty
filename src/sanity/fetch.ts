...
export async function getPropertyById(id: string): Promise<Property | undefined> {
  ...
  try {
    ...
    const data = await client.fetch(propertyByIdQuery, { id }, { next: { revalidate: 60 } });
...
export async function getPropertiesByArea(area: string): Promise<Property[]> {
  ...
  try {
    ...
    const data = await client.fetch(propertiesByLocationQuery, { area }, { next: { revalidate: 60 } });
...
export async function getProjectBySlug(slug: string): Promise<Project | undefined> {
  ...
  try {
    ...
    const data = await client.fetch(projectBySlugQuery, { slug }, { next: { revalidate: 60 } });
...
export async function getArticleBySlug(slug: string): Promise<Article | undefined> {
  ...
  try {
    ...
    const data = await client.fetch(articleBySlugQuery, { slug }, { next: { revalidate: 60 } });
...
export async function getAgents(): Promise<Agent[]> {
  ...
  try {
    const client = getSanityClient();
    if (!client) return [];
    const data = await client.fetch(allAgentsQuery, {}, { next: { revalidate: 60 } });
...
export async function getLocations() {
  ...
  try {
    const client = getSanityClient();
    if (!client) return [];
    return await client.fetch(allLocationsQuery, {}, { next: { revalidate: 60 } });
  } catch {
    return [];
  }
}
...
export async function getStays(): Promise<Stay[]> {
  ...
  try {
    ...
    const data = await client.fetch(allStaysQuery, {}, { next: { revalidate: 60 } });
